...
**Sign in → Budget → Invest → Retire.**

1. **Budget** (`/budget`) — leftover, envelopes, register, Plaid Connect, credit-card payment envelopes. Empty offers the first-run kit.
2. **Invest** (`/invest`) — the public-stock book. Search a name or ticker. Score sits above Past / Now / Future. Empty offers the first-book wizard. An existing book is never hidden or deleted.
3. **Retire** (`/retire`) — one date from leftover and the book. Target, on-track, and the lever. A date still needs leftover and the book.

Learn/Do tabs, Journey Home, and the Money Profile quiz are unshipped. `/home` and `/money-profile` redirect to Budget. `/freedom` redirects to Retire. `/chat` and `/assistant` redirect to Invest. Chat is unshipped. Do not remount it.
...
- Signed-in `/` goes to Budget.
- Logo click when signed in goes to Budget. Logo when signed out goes to `/`.
- Signed-in header is Logo, Budget | Invest | Retire, and an account menu with Settings and Sign out.
